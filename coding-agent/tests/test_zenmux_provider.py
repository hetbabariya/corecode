"""Standalone ZenMux API validation — run before integrating into core.

Usage:
    uv run python tests/test_zenmux_provider.py

Requires:
    - ZENMUX_API_KEY in environment or .env
    - openai package installed (already a dependency via cerebras-cloud-sdk)
"""
...
def _get_api_key() -> str:
    ...
    if not key:
        try:
            ...
            pass
    if not key:
        print("ERROR: Set ZENMUX_API_KEY in env or .env file")
        raise SystemExit(1)
    return key


def _make_client() -> AsyncOpenAI:
    return AsyncOpenAI(base_url=BASE_URL, api_key=_get_api_key())
